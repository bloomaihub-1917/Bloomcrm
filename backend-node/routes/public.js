...
/* ── 로고 ──────────────────────────────────────────────────────
   ...
   못 읽는다. db/import-directory-logos.js가 래스터만 골라 웹용으로 줄여
   여기에 넣는다(파일명은 도록 순번 — 로고 폴더의 번호가 아니다. 두 번호는
   어긋나서, 그쪽 스크립트가 이름으로 짝지어 정한다).
...
const LOGO_ROOT = path.join(__dirname, '..', 'assets', 'logos');
